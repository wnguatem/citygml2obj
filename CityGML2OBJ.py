...
def convert(infile, outfile):
    ...
    # loop through the Building elements, for all faces put the points in a set (so that they are not duplicated). Then extend the pointlist with that set. This gives every unique point an index
    for b in tree.iter(CGML+"Building"):
        s = set([])
        for lR in b.iter(GML+"LinearRing"):
            for pos in lR:
                s.update([pos.text])
                #print >>vOut, 
        pointlist += list(s)
    ...
    # write the file
    f = open(OUTFILE, 'w')
    f.write("# location in WGS84 (lat2long): 51.9985,4.374211\n") # specific for the tu-delt capus
